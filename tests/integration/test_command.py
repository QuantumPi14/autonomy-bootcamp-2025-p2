...
def read_queue(
    args: tuple,  # Add any necessary arguments (output_queue, controller)
    main_logger: logger.Logger,
) -> None:
    """
    Read and print the output queue.
    """
    output_queue, controller = args[0], args[1]
    while not controller.is_exit_requested():
        try:
            msg = output_queue.queue.get(timeout=0.5)
...
def put_queue(
    args: tuple,  # Add any necessary arguments (input_queue, path, controller)
) -> None:
    """
    Place mocked inputs into the input queue periodically with period TELEMETRY_PERIOD.
    """
    input_queue, path, controller = args[0], args[1], args[2]
    for item in path:
...
def main() -> int:
    ...
    # Create your queues
    input_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, 0)
    output_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, 0)
    ...
    threading.Thread(
        target=put_queue,
        args=((input_queue, path, controller),),
    ).start()
